...
export default function TopHeader({ setView }: { setView: (v: string) => void }) {
  const items = [
    { label: 'Verify', onClick: () => setView('verify'), icon: <ShieldCheck size={16} /> },
    { label: 'Report', onClick: () => setView('capture'), icon: <Plus size={16} /> }
  ];
  ...
  return (
    <header className="h-16 border-b border-border-default bg-surface-card flex items-center justify-between flex-shrink-0 relative z-40 w-full pl-6 md:pl-8 pr-[100px] md:pr-[120px]">
      ...
      <div className="flex items-center gap-6 w-auto flex-shrink-0">
        <div className="flex items-center gap-3 cursor-pointer select-none" onClick={() => setView('dashboard')}>
          <div className="w-8 h-8 rounded-lg bg-accent flex items-center justify-center flex-shrink-0">
            <span className="text-text-on-accent font-black text-lg leading-none">N</span>
          </div>
      ...
      <div className="flex items-center justify-end gap-3 md:gap-6 flex-shrink-0">
        <ThemeToggle />
        <button onClick={() => setView("alerts")} className="relative text-text-muted hover:text-text-primary transition-colors shrink-0">
          <Bell className="w-5 h-5" />
          <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-danger rounded-full border-2 border-surface-card" />
        </button>
        
        <div onClick={() => setView("profile")} className="flex items-center gap-2 cursor-pointer hover:bg-surface-muted p-1.5 rounded-lg transition-colors shrink-0">
          <img
          ...
          <div className="hidden xl:flex flex-col text-left">
            <span className="text-sm font-semibold text-text-primary leading-tight">Rohan Kumar</span>
            <span className="text-xxs text-text-muted leading-tight">Citizen</span>
          </div>
